...
def sync_order_status(order_name: str) -> str:
    """Synchronize generic order lifecycle status without encoding route position."""
    open_replacements = (
        frappe.db.count(
            "Replacement Piece",
    ...
    if open_replacements:
        status = derive_order_status(current_status=None, production_path=None, current_stage=None, stages=(), has_open_replacements=True)
    ...
    if current and current.status in {"Ready for Delivery", "Delivered", "Cancelled"}:
        return current.status

    if current and current.production_path and current.current_production_stage:
        stage = frappe.db.get_value("Production Stage", current.current_production_stage, ["stage_type", "status"], as_dict=True)
        if stage and stage.status != "Cancelled":
            frappe.db.set_value("Door Cutting Order", order_name, "status", PRODUCTION_ORDER_STATUS, update_modified=True)

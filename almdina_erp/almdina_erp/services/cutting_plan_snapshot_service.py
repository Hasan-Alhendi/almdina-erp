...
def create_plan_from_order(
    ...
    *,
    ...
    if not validation.get("is_valid"):
        frappe.throw(
            _(
                "The cutting plan failed geometry validation and cannot be approved."
            )
        )
    if unplaced:
        ...
        if not settings.allow_unplaced_approval:
            frappe.throw(
                _(
                    "The cutting plan contains unplaced pieces and cannot be approved."
                )
            )
    ...
    plan.optimization_mode = (
        snapshot.get("optimization_mode") or order.packing_mode or ""
    )
    plan.machine_type = (
        snapshot.get("machine_type")
        or order.cutting_machine_type
        or "Auto"
    )
    plan.method_key = snapshot.get("method_key") or ""
    plan.method_label = (
        snapshot.get("method_label") or order.packing_method or ""
    )
    plan.ordering_strategy = snapshot.get("ordering_strategy") or ""
    plan.score = flt(snapshot.get("score"))
    plan.engine_version = (
        snapshot.get("engine_version") or order.engine_version or ""
    )
    ...
    plan.estimated_cut_length_m = (
        flt(metrics.get("estimated_cut_length_cm")) / 100
    )
    ...
    plan.usable_width_mm = usable_width_mm
    plan.usable_length_mm = usable_length_mm
    ...
    for sheet in snapshot.get("sheets") or []:
        ...
        used_area = sum(
            flt(piece.get("area_m2")) for piece in sheet_pieces
        )
        source_full_width_mm = flt(
            sheet.get("full_width_cm")
            or snapshot.get("full_board_width_cm")
        ) * 10
        source_full_length_mm = flt(
            sheet.get("full_length_cm")
            or snapshot.get("full_board_length_cm")
        ) * 10
        source_usable_width_mm = flt(
            sheet.get("usable_width_cm")
            or sheet.get("w")
            or snapshot.get("usable_board_width_cm")
        ) * 10
        source_usable_length_mm = flt(
            sheet.get("usable_length_cm")
            or sheet.get("h")
            or snapshot.get("usable_board_length_cm")
        ) * 10
        source_area = flt(sheet.get("source_area_m2")) or (
            source_usable_width_mm
            * source_usable_length_mm
            / 1_000_000
        )
        ...
        for piece in sheet_pieces:
            ...
            source_row = (
                piece_groups[source_no - 1]
                if 0 < source_no <= len(piece_groups)
                else None
            )
            effective_edge_type = ""
            if source_row:
                effective_edge_type = (
                    source_row.edge_type
                    or order.default_edge_type
                    or ""
                )
            ...
            plan.append(
                "placed_pieces",
                {
                    ...
                    "clipped_corner_position": (
                        piece.get("clipped_corner_position") or ""
                    ),
                    "clipped_corner_width_cm": flt(
                        piece.get("clipped_corner_width_cm")
                    ),
                    "clipped_corner_length_cm": flt(
                        piece.get("clipped_corner_length_cm")
                    ),
                    ...
                    "edge_long_right": (
                        1 if piece.get("edge_long_right") else 0
                    ),
                    "edge_long_left": (
                        1 if piece.get("edge_long_left") else 0
                    ),
                    "edge_width_top": (
                        1 if piece.get("edge_width_top") else 0
                    ),
                    "edge_width_bottom": (
                        1 if piece.get("edge_width_bottom") else 0
                    ),
...
def approve_plan(plan: Any) -> Any:
    ...
    if plan.validation_status != "Valid":
        frappe.throw(
            _("Only a geometrically valid cutting plan can be approved.")
        )
...
def lock_order_for_production(
    ...
    *,
    ...
    """Freeze the selected validated snapshot as the production Cutting Plan."""
    ...
    if plan_source == "Custom":
        ...
        if not custom_raw:
            frappe.throw(
                _(
                    "Upload and import a revised DXF before approving the custom plan."
                )
            )
        if not order.production_dxf:
            frappe.throw(
                _("Upload a revised DXF before approving the custom plan.")
            )
    ...
    if (
        not get_system_plan_json(order)
        and order.cutting_plan_json
        and has_dual_plan_field("system_plan_json")
    ):
        frappe.db.set_value(
            "Door Cutting Order",
            order.name,
    ...
    update_values: dict[str, Any] = {
        ...
        "packing_score": _(
            "Full boards: {0} | Waste: {1}% | Method: {2}"
        ).format(
            plan.required_boards,
            plan.waste_percent,
            plan.method_label,
        ),
    ...
    if preserve_status:
        update_values["drawing_dxf_status"] = (
            "Approved by Drawing"
            if plan_source == "Custom"
            else (order.drawing_dxf_status or "None")
        )
    else:
    ...
    # Shop-floor stages are created only by the explicit dispatch command.
    return {
        "name": order.name,
        "status": "Approved",

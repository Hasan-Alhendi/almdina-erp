frappe.pages["shop-floor-inbox"].on_page_load = function (wrapper) {
    ...
    function loadList() {
        ...
        const method = requestedMode === "archive" ? METHODS.archive : METHODS.inbox;
        return frappe
            .call({ method, freeze: false })
            .then(response => {
                if (requestId !== listRequest || requestedMode !== mode) return;
                renderList(response.message || []);
            })
            .catch(error => {
                if (requestId !== listRequest || requestedMode !== mode) return;
    ...
    function buildActionsHtml(detail, meta) {
        ...
        if (mode === "inbox" && detail.can_start_stage) {

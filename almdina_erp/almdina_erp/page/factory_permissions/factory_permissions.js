frappe.pages["factory-permissions"].on_page_load = function (wrapper) {
    ...
    function renderShell() {
        $main.html(`
            <div class="apc-shell">
                <div class="apc-hero">
                    <div><h2>${__("مصفوفة صلاحيات Almdina")}</h2><p>${__("امنح كل دور ما يحتاجه فقط. تعتمد الواجهة والخدمات والبيانات على هذه المصفوفة، بينما تبقى أدوار الأقسام التشغيلية مخصصة لأهلية الإسناد فقط.")}</p></div>
    ...
    function loadPreview() {
        ...
        return frappe.call({
            ...
            args: { role: state.selectedRole, capabilities: JSON.stringify(requestedState) },
            freeze: false,
        }).then(
            response => {
                if (requestId !== state.previewRequest || stable(requestedState) !== stable(state.working)) return null;
    ...
    function renderImpact(data) {
        ...
        $main.find(".apc-impact-panel").html(`<div class="apc-panel-title">${__("أثر الصلاحيات")}</div>${warning}<div style="margin-top:10px;font-size:12px"><span class="text-muted">${__("الصفحة الرئيسية")}</span><br><b>${esc(home)}</b></div><div style="margin-top:12px" class="apc-panel-title">${__("مساحات العمل")}</div><div class="apc-chip-row">${workspaces.length ? workspaces.map(item => `<span class="apc-chip">${esc(item)}</span>`).join("") : `<span class="text-muted">${__("لا توجد مساحة إضافية")}</span>`}</div><div style="margin-top:12px" class="apc-panel-title">${__("الأقسام الظاهرة")}</div><div class="apc-chip-row">${enabledSections.length ? enabledSections.map(key => `<span class="apc-chip">${esc(labels[key] || key)}</span>`).join("") : `<span class="text-muted">${__("لا توجد أقسام")}</span>`}</div><div style="margin-top:14px" class="apc-panel-title">${__("التغييرات الحالية")}</div>${changeHtml}`);
    }

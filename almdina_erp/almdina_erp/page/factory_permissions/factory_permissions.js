frappe.pages["factory-permissions"].on_page_load = function (wrapper) {
    ...
    const METHODS = Object.freeze({
        ...
        template: "almdina_erp.almdina_erp.services.permission_management_service.preview_permission_template",
    ...
    function renderShell() {
        $main.html(`
            <div class="apc-shell">
                <div class="apc-hero">
                    <div><h2>${__("مصفوفة صلاحيات Almdina")}</h2><p>${__("امنح كل دور ما يحتاجه فقط. القوالب اختيارية ولا تحفظ شيئًا تلقائيًا، والاستيراد يعرض المعاينة قبل الحفظ النهائي.")}</p></div>
                ...
                <div class="apc-toolbar">
                    ...
                    <div class="apc-panel"><div class="apc-panel-title">${__("قالب أو نقل الصلاحيات")}</div><div class="apc-template-tools"><select class="apc-select apc-template-select" aria-label="${__("قالب الصلاحيات")}"></select><button type="button" class="btn btn-default apc-apply-template">${__("تطبيق للمعاينة")}</button></div><div class="apc-transfer-tools"><button type="button" class="btn btn-default apc-export">${__("تصدير JSON")}</button><button type="button" class="btn btn-default apc-import">${__("استيراد JSON")}</button></div><input type="file" class="apc-import-file" accept="application/json,.json" hidden></div>
    ...
    function bindEvents() {
        ...
        $main.on("click", ".apc-apply-template", applySelectedTemplate);
    ...
    function renderTemplateOptions() {
        const $select = $main.find(".apc-template-select").empty();
        $("<option>").attr("value", "").text(__("اختر قالبًا اختياريًا...")).appendTo($select);
        state.templates.forEach(template => {
            $("<option>").attr("value", template.key).text(template.label).appendTo($select);
        });
    }
    ...
    function renderCapability(capability) {
        const risk = capability.risk === "critical" ? `<span class="apc-badge critical">${__("حرجة")}</span>` : capability.risk === "sensitive" ? `<span class="apc-badge sensitive">${__("حساسة")}</span>` : "";
    ...
    function orderCapabilityKeys() {
        return state.catalog.flatMap(group => group.capabilities || []).filter(item => item.doctype === "Door Cutting Order").map(item => item.key);
    }
    ...
    function onCapabilityChange() {
        ...
        if (key === "view_orders" && state.working[key] !== true) {
            const dependent = orderCapabilityKeys().some(item => item !== "view_orders" && state.working[item] === true);
            if (dependent) {
                state.working.view_orders = true;
                $(this).prop("checked", true);
                frappe.show_alert({ message: __("عرض الطلبات مطلوب تلقائيًا عند منح أي صلاحية على الطلب."), indicator: "orange" });
            }
        } else if (key !== "view_orders" && state.working[key] === true && orderCapabilityKeys().includes(key)) {
            state.working.view_orders = true;
    ...
    function applySelectedTemplate() {
        const templateKey = String($main.find(".apc-template-select").val() || "");
        if (!templateKey || !state.selectedRole) {
            frappe.show_alert({ message: __("اختر قالبًا أولًا."), indicator: "orange" });
            return;
        }
        const template = state.templates.find(item => item.key === templateKey) || {};
        const proceed = () => previewExternal(METHODS.template, { role: state.selectedRole, template_key: templateKey }, __("تم تطبيق القالب للمعاينة فقط. راجع التغييرات ثم اضغط حفظ."));
        frappe.confirm(__("سيستبدل القالب الصلاحيات الحالية في شاشة المعاينة للدور {0}. لن يتم الحفظ تلقائيًا. متابعة؟", [state.selectedRole]), proceed);
        if (template.description) frappe.show_alert({ message: template.description, indicator: template.risk === "critical" ? "orange" : "blue" }, 5);
    }
    ...
    function renderImpact(data) {
        ...
        const source = data && data.source ? `<div class="apc-source">${data.source.kind === "template" ? __("المصدر: قالب صلاحيات اختياري") : __("المصدر: ملف مستورد من الدور {0}", [esc(data.source.role || "—")])}</div>` : "";
        ...
        $main.find(".apc-impact-panel").html(`<div class="apc-panel-title">${__("أثر الصلاحيات")}</div>${source}${warning}<div style="margin-top:10px;font-size:12px"><span class="text-muted">${__("الصفحة الرئيسية")}</span><br><b>${esc(home)}</b></div><div style="margin-top:12px" class="apc-panel-title">${__("مساحات العمل")}</div><div class="apc-chip-row">${workspaces.length ? workspaces.map(item => `<span class="apc-chip">${esc(item)}</span>`).join("") : `<span class="text-muted">${__("لا توجد مساحة إضافية")}</span>`}</div><div style="margin-top:12px" class="apc-panel-title">${__("الأقسام الظاهرة")}</div><div class="apc-chip-row">${enabledSections.length ? enabledSections.map(key => `<span class="apc-chip">${esc(labels[key] || key)}</span>`).join("") : `<span class="text-muted">${__("لا توجد أقسام")}</span>`}</div><div style="margin-top:14px" class="apc-panel-title">${__("التغييرات الحالية")}</div>${changeHtml}`);
    }
    ...
    function syncDirtyState() {
        const dirty = isDirty();
        $main.find(".apc-dirty").toggleClass("is-dirty", dirty).text(dirty ? __("توجد تغييرات غير محفوظة") : __("لا توجد تغييرات غير محفوظة"));
        $main.find(".apc-save,.apc-reset").prop("disabled", !dirty || state.saving);
        $main.find(".apc-apply-template,.apc-export,.apc-import").prop("disabled", state.saving || !state.selectedRole);
    }
    ...
    function persistPermissions(confirmSelfLockout) {
        ...
        frappe.call({
            ...
            error => frappe.msgprint({ title: __("تعذر حفظ الصلاحيات"), message: esc(error && error.message ? error.message : __("حدث خطأ غير متوقع.")), indicator: "red" })
        ).then(() => {
            state.saving = false;
            syncDirtyState();
        });

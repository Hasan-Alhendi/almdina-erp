(() => {
    ...
    if (!api || !editorFactory || !persistence || !projection) throw new Error("Professional workspace dependencies are incomplete");
    ...
    function mount(wrapper) {
        ...
        let generation = 0;
        let suspended = false;
        function destroyEditor(){if(editor){editor.destroy();editor=null;}}
        ...
        async function open(route){const token=++generation;suspended=false;showLoading();try{const loaded=await api.load(route.orderName,route.pieceName);if(token!==generation||suspended)return;context=loaded;const document=persistence.fromStored(loaded.piece.special_shape_drawing_json,loaded.piece);main.innerHTML="";editor=editorFactory.create({container:main,document,readOnly:!loaded.permissions.can_edit,meta:{orderName:loaded.order.name,pieceLabel:`درفة ${loaded.piece.piece_no||""}`},onSave:save,onBack:requestBack});if(!loaded.permissions.can_edit&&loaded.permissions.edit_reason)frappe.show_alert({message:loaded.permissions.edit_reason,indicator:"orange"},5);}catch(error){console.error("Professional drawing workspace load failed",error);if(token===generation)showRouteError("تعذر تحميل الدرفة. افتح الرسم من الطلب وتحقق من الصلاحيات.");}}

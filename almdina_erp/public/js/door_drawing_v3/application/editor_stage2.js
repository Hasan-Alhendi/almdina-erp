(() => {
    ...
    function resolvedArcEndpoint(c, event, center, radiusMm, options = {}) {
        const result = S.resolveArcEndpoint(
            c.history.current(),
            V.eventWorld(c, event),
            center,
            radiusMm,
            { viewportScale: c.viewport.scale, excludeId: options.excludeId }
        );
        c.snapState = result;
        return result.point;
    }
    ...
    function handleArcClick(c, event) {
        ...
        const sweep = G.normalizeAngle(endAngle - c.arcDraft.startAngleDeg);
    ...
    function beginPan(c, event) {
        ...
        c.gesture = { type: "pan", pointerId: event.pointerId, x: p.x, y: p.y, ox: c.viewport.offsetX, oy: c.viewport.offsetY };
        c.canvas.classList.add("is-panning");
    ...
    function pointerDown(c, event) {
        ...
        if (handle && c.selectedId) {
            const object = selected(c);
            const descriptor = object && Handles.handleByRole(object, handle.dataset.ddv3Handle);
            if (!object || !descriptor) return;
            clearSnap(c);
            c.gesture = { type: "handle", pointerId: event.pointerId, role: descriptor.role, descriptor, object };
    ...
    function handleCandidate(c, gesture, event) {
        const object = gesture.object;
        const role = gesture.role;
        const descriptor = gesture.descriptor;
        if (object.type === "line") {
            const anchor = role === "start" ? object.geometry.end : object.geometry.start;
            return resolvedCandidate(c, event, anchor, { axisLock: true, excludeId: object.id });
        }
        if (object.type === "rectangle") {
            if (event.shiftKey) { clearSnap(c); return V.eventWorld(c, event); }
            return resolvedCandidate(c, event, null, { excludeId: object.id });
        }
        if (object.type === "circle") {
            if (role === "center") return resolvedCandidate(c, event, null, { excludeId: object.id });
            return resolvedCandidate(c, event, object.geometry.center, { forcedAxis: descriptor.axis, excludeId: object.id });
        }
        if (object.type === "arc") {
            if (role === "center") return resolvedCandidate(c, event, null, { excludeId: object.id });
            if (role === "start" || role === "end") return resolvedArcEndpoint(c, event, object.geometry.center, object.geometry.radiusMm, { excludeId: object.id });
            clearSnap(c);
            return V.eventWorld(c, event);
        }
        clearSnap(c);
        return V.eventWorld(c, event);
    }
    ...
    function applyInspector(c, input) {
        if (c.readOnly || !input || input.disabled) return;
        const object = selected(c);
        if (!object) return;
        const key = input.dataset.ddv3Prop, value = G.number(input.value, NaN);
        if (!Number.isFinite(value)) return;
        let next = object, g = object.geometry;
        try {
            if (object.type === "line") {
                if (key === "length") next = G.resizeLine(object, value);
                else if (key === "angle") next = G.resizeLine(object, G.lineLength(object), value);
                else if (key === "x" || key === "y") next = G.translateObject(object, key === "x" ? value - g.start.x : 0, key === "y" ? value - g.start.y : 0);
                else return;
            } else if (object.type === "rectangle") {
                next = G.setRectangle(object, { [key === "width" ? "widthMm" : key === "height" ? "heightMm" : key]: value });
            } else if (object.type === "circle") {
                next = G.setCircle(object, { [key === "radius" ? "radiusMm" : key === "diameter" ? "radiusMm" : key]: key === "diameter" ? value / 2 : value });
            } else if (object.type === "arc") {
                next = G.setArc(object, { [key === "radius" ? "radiusMm" : key === "startAngle" ? "startAngleDeg" : key === "sweep" ? "sweepAngleDeg" : key]: value });
            }
            clearSnap(c);
    ...
    function zoomAt(c, factor, x, y) {
        clearSnap(c);
        const old = c.viewport.scale, next = Math.max(V.MIN_SCALE, Math.min(V.MAX_SCALE, old * factor));
        if (Math.abs(next - old) < 1e-9) return;
        const world = V.screenToWorld(c, x, y);
    ...
    function bind(c) {
        c.root.addEventListener("click", event => {

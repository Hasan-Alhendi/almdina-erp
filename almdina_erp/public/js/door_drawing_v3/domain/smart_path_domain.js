(() => {
    ...
    function path(id, points, closed = false, style = {}) {
        ...
        return Object.freeze({
            ...
            geometry: Object.freeze({
                points: Object.freeze(safePoints.map(p => BaseG.point(p.x, p.y))),
                closed: isClosed,
            }),
            style: BaseG.styleObject ? BaseG.styleObject(style) : Object.freeze({
                stroke: String(style.stroke || "#1e1e1e"),
                strokeWidthMm: Math.max(0.05, BaseG.roundMm(style.strokeWidthMm || 0.35)),
            }),
        });
    }
    ...
    function pathSegments(object) {
        ...
        for (let index = 0; index < points.length - 1; index += 1) {
            segments.push(Object.freeze({ index, start: points[index], end: points[index + 1] }));
        }
        if (object.geometry.closed && points.length > 2) {
            segments.push(Object.freeze({ index: points.length - 1, start: points[points.length - 1], end: points[0] }));
        }

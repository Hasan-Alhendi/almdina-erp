(() => {
    ...
    const STYLE_ID = "dco-multi-edge-documents-css";
    let activeFrm = null;
    ...
    function sideText(detail) {
        return detail.sides && detail.sides.length ? detail.sides.join(" + ") : "—";
    }
    ...
    function edgeDetailsHtml(details, { prices = false } = {}) {
        ...
        return `<div class="dco-edge-detail-list">${details.map(detail => `
            <div class="dco-edge-detail-badge">
                <b>${esc(detail.axis_label)} · ${esc(sideText(detail))}</b>
    ...
    function edgeInvoiceLines(frm) {
        ...
        sourceRows.forEach(row => {
            row.details.forEach(detail => {
                ...
                const key = `${detail.axis}::${detail.edge_type}::${detail.rate}::${detail.thickness_mm}`;
                if (!groups.has(key)) {
                    groups.set(key, {
                        type: "edge",
                        description: `${detail.axis === "long" ? "قشاط طولي" : "قشاط عرضي"} — ${detail.edge_type || "غير محدد"}`,
                        quantity: 0,
                        unit: "متر",
                        rate: detail.rate,
                        amount: 0,
                        note: `السماكة ${quantity(detail.thickness_mm)} مم — الجهات ${sideText(detail)}`,
                    });
                }
        ...
        return [...groups.values()].map(line => ({
            ...
            amount: Math.round(line.amount * 1000) / 1000,
        }));
    }
    ...
    function installStyles() {
        ...
        style.textContent = `
            ...
            .dco-edge-detail-badge b{font-size:10px}.dco-edge-detail-badge span{font-size:9px;color:var(--text-muted,#66717e);word-break:break-word}
    ...
    function patchMeasurements(frm, wrapper) {
        ...
        if (headerCells[5]) headerCells[5].textContent = "تفاصيل القشاط";
        const data = rows(frm);
        ...
        if (subtitle) subtitle.textContent = "كل محور يعرض نوع القشاط والجهات المستخدمة بوضوح للعامل";
    }
    ...
    function patchInvoice(frm, wrapper) {
        ...
        if (tbody.dataset.multiEdgeInvoiceSignature !== signature) {
            tbody.dataset.multiEdgeInvoiceSignature = signature;
            tbody.innerHTML = invoiceRowsHtml(frm);
        }
        const subtitle = section.querySelector(".dco-cost-section-title span");
        if (subtitle) {
            subtitle.innerHTML = '<span class="dco-edge-pricing-note">سطر مستقل لكل نوع قشاط وسعره وأمتاره</span>';
        }
    ...
    function patchCostScreen(frm) {
        ...
        if (!wrapper._dcoMultiEdgeDocumentsObserver) {
            let queued = false;
            ...
            wrapper._dcoMultiEdgeDocumentsObserver = observer;
        }
    }
    ...
    function printInvoiceHtml(frm) {
        ...
        return `<!doctype html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>فاتورة الطلب ${esc(frm.doc.name || "")}</title><style>
            ...
            <table class="table measurements"><thead><tr><th>#</th><th>النوع</th><th>العرض</th><th>الطول</th><th>العدد</th><th>تفاصيل القشاط</th><th>ملاحظات</th></tr></thead><tbody>${printRowsHtml(frm)}</tbody></table>
    ...
    function printMeasurementsHtml(frm) {
        const generated = frappe.datetime ? frappe.datetime.now_datetime() : new Date().toISOString();
        return `<!doctype html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>قياسات الطلب ${esc(frm.doc.name || "")}</title><style>
            @page{size:A4 portrait;margin:10mm}*{box-sizing:border-box}body{font-family:Tahoma,Arial,sans-serif;color:#111;margin:0;font-size:9px;direction:rtl}.header{display:flex;justify-content:space-between;border-bottom:2px solid #111;padding-bottom:8px;margin-bottom:9px}.header h1{font-size:20px;margin:0 0 3px}.muted{color:#666;font-size:8px}.table{width:100%;border-collapse:collapse}.table th,.table td{border:1px solid #999;padding:4px;text-align:center;vertical-align:middle}.table th{background:#eee}.right{text-align:right!important;white-space:normal}.dco-edge-detail-list{display:grid;gap:2px}.dco-edge-detail-badge{display:grid;gap:1px;padding:2px 4px;border:1px solid #bbb;border-radius:3px;text-align:right}.dco-edge-detail-badge b{font-size:7.8px}.dco-edge-detail-badge span{font-size:7px;color:#444}.dco-edge-none{color:#666}.dimension{display:inline-flex;min-width:36px;flex-direction:column;align-items:center;gap:1px}.edge-lines{display:flex;flex-direction:column;gap:1px;min-height:4px}.edge-line{display:block;width:26px;height:1px;background:#111}.edge-lines-0{visibility:hidden}.footer{margin-top:10px;border-top:1px solid #aaa;padding-top:5px;display:flex;justify-content:space-between;color:#666;font-size:8px}
        </style></head><body>
            <div class="header"><div><h1>جدول قياسات الطلب</h1><div class="muted">تفصيل نوع القشاط لكل محور وجهاته</div></div><div style="text-align:left"><b>${esc(frm.doc.name || "مسودة")}</b><div class="muted">${esc(frm.doc.order_date || "")}</div></div></div>
            <table class="table"><thead><tr><th>#</th><th>النوع</th><th>العرض</th><th>الطول</th><th>العدد</th><th>تفاصيل القشاط</th><th>ملاحظات</th></tr></thead><tbody>${printRowsHtml(frm)}</tbody></table>
    ...
    function printHtml(html, frameId) {
        ...
        frame.onload = () => {
            try {
                ...
                console.error("Multi edge print failed", error);
                cleanup();
    ...
    function bindPrintInterception() {
        if (document._dcoMultiEdgePrintBound) return;
        document._dcoMultiEdgePrintBound = true;
        document.addEventListener("click", event => {
            ...
            if (invoice) {
                ...
                printHtml(printInvoiceHtml(activeFrm), "dco-multi-edge-invoice-frame");
                return;
            }
            ...
            if (measurements) {
                event.preventDefault();
                event.stopImmediatePropagation();
                printHtml(printMeasurementsHtml(activeFrm), "dco-multi-edge-measurements-frame");
            }
        }, true);
    }
    ...
    window.AlmdinaMultiEdgeDocuments = {
        ...
        printInvoice(frm) { printHtml(printInvoiceHtml(frm), "dco-multi-edge-invoice-frame"); },
        printMeasurements(frm) { printHtml(printMeasurementsHtml(frm), "dco-multi-edge-measurements-frame"); },
    };
})();

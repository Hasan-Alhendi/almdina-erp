...
def test_all_new_ux_layers_keep_their_correct_eager_or_lazy_owner():
    hooks = text(HOOKS)
    ...
    # Measurement entry and toolbar behavior are first-open Order concerns.
    assert measurement in hooks
    assert toolbar in hooks
    assert hooks.index(measurement) < hooks.index(toolbar)

    # DXF export is Plan-only and must stay off the initial form critical path.

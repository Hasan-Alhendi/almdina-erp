...
def test_contextual_edit_action_is_the_final_page_toolbar_owner() -> None:
    assets = _dco_assets()
    coordinator = (
    ...
    assert assets.index(plan_session) < assets.index(plan_access)
    assert assets.index(plan_access) < assets.index(coordinator)
    assert assets.rstrip().endswith(coordinator + ",")

...
class TestPermissionManagementArchitecture(unittest.TestCase):
    ...
    def test_pages_and_settings_doctype_have_no_fixed_roles(self) -> None:
        ...
        settings_page = json.loads(
            SETTINGS_PAGE_JSON.read_text(encoding="utf-8")
        )
        settings_doctype = json.loads(
            SETTINGS_DOCTYPE.read_text(encoding="utf-8")
        )
    ...
    def test_audit_is_append_only_and_not_directly_exposed(self) -> None:
        ...
        controller = AUDIT_DOCTYPE.with_suffix(".py").read_text(
            encoding="utf-8"
        )
    ...
    def test_settings_workspace_has_direct_administration_entries(self) -> None:
        workspace = json.loads(
            SETTINGS_WORKSPACE.read_text(encoding="utf-8")
        )
        targets = {
        ...
        self.assertEqual(
            targets["إدارة مسارات الإنتاج"],
            "factory-master-data",
        )
        self.assertIn(
            'routes: ["factory-permissions", "role"]',
            SHARED_SHELL.read_text(encoding="utf-8"),
        )

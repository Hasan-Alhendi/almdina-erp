...
def sync_setup() -> None:
    """Synchronize product data without inventing roles, users, or routes."""
...
def seed_settings_defaults() -> None:
    ...
    for fieldname, value in defaults.items():
        if settings.get(fieldname) in (None, ""):
            ...
            changed = True
    # A default route is selected only after an administrator creates one.
    # Inventory, reservations, consumption and remnant reuse are outside the
    # current product; keep historical rows but disable those controls.
...
def sync_plan_recalculation_state() -> None:
    """Keep existing valid plans usable after adding the freshness flag."""
...
def sync_dual_plan_json_backfill() -> None:
    """Copy existing cutting plans into system_plan_json after dual-plan fields land."""

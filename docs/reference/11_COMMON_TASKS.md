...
5. راجع الأرقام قبل الحفظ، خصوصًا القياسات والكمية. لا حاجة لمغادرة الخلية يدويًا؛ الحفظ يثبت القيمة المفتوحة.
6. احفظ الطلب. زر الحفظ يجب أن يتحول إلى «تعديل» من أول نقرة إذا نجح التثبيت. إذا كانت التغييرات تؤثر على توزيع القطع وكنت تملك `recalculate_plan`، يبدأ حساب خطة النظام والتكلفة في الخلفية دون تجميد الشاشة. بعد أول حفظ، وبعد كل حفظ في تاب الطلب، يظهر سؤال إرسال جدول القياسات عبر واتساب إن كانت الجلسة متصلة وكنت تملك `print_measurements`. سؤال الحفظ الأول يطلب إرسال ملف القياسات، وسؤال الحفظ اللاحق يطلب إرسال تعديلات القياسات. إذا أُغلق السؤال دون إرسال، يبقى زر في رأس المسودة (بجانب أزرار الصفحة الحالية) طالما التعديل ممكناً: «إرسال القياسات عبر واتساب» أو «إرسال تعديلات القياسات عبر واتساب». الضغط على الزر يعيد فتح نفس سؤال التأكيد؛ إن كان الطلب غير محفوظ يُطلب الحفظ أولاً. إذا كانت الجلسة غير متصلة تظهر رسالة تحذير دون إرسال. الرقم يُقرأ من `Customer.mobile_no` ويُحوَّل إلى صيغة سورية `+963` مع حذف صفر البداية. الملف المرسل هو نفس جدول القياسات المستخدم في الطباعة، ويُنشأ PDF على الخادم عبر Chromium؛ امتلاء القرص يمنع الإنشاء. النص يُرفق مع ملف PDF في رسالة واحدة (حد 1024 حرفًا) ويُضبط من إعدادات المعمل → رسائل واتساب: رسالة القياسات لأول إرسال ورسالة تعديلات القياسات بعد ذلك (`{order_name}` = رقم الطلب). لإرسال فاتورة الزبون افتح تاب التكلفة واضغط «إرسال الفاتورة عبر واتساب» بعد التأكيد؛ يستخدم قالب رسالة الفاتورة من القسم نفسه، ويتطلب `print_customer_invoice`.
7. إذا كان هناك رسم خاص، انتقل لمسار الرسم المخصص بدل كتابة وصف غامض فقط.
8. راجع Cutting Plan بعد اكتمال الحساب التلقائي، أو شغّل إعادة الحساب اليدوي إذا ظهر أن العامل الخلفي لم يبدأ، قبل Dispatch إلى الإنتاج.

### النتيجة المتوقعة

- DCO محفوظ بهوية ثابتة.
- القياسات والقشاط موجودان كمصدر التخطيط.
- خطة النظام والتكلفة المرتبطة بعدد الألواح تتحدث تلقائيًا بعد الحفظ عندما تسمح الصلاحية والحالة بذلك. خطة DXF المرفوعة تُعلَّم بأنها قديمة حتى يُرفع ملف جديد أو تُعتمد خطة النظام.
...
2. افتح «توثيق الدرفة الخاصة» من سطر الدرفة؛ تفتح الصفحة في تاب جديدة ويبقى الطلب في التاب الأصلية. زر الرجوع يغلق تاب التوثيق.
...
إذا كان الحساب الخلفي ما زال جاريًا، انتظر اكتماله. إذا بقي الشريط على الانتظار دون أن يبدأ العامل، استخدم زر إعادة الحساب اليدوي بعد التأكد أن `bench worker` وطابور `long` يعملان. لا تُكمل الإرسال للإنتاج بالإجبار على خطة قديمة.
...
5. إذا أردت إشعار الزبون عند إتمام مرحلة، فعّل مفتاح **إرسال واتساب عند الإتمام** ثم اضبط نص الرسالة من إعدادات المعمل → رسائل واتساب. ربط الجلسة يحتاج `manage_whatsapp_session`، وتعديل النصوص يحتاج `edit_whatsapp_messages`.
6. احفظ واختبر route على طلب تجريبي قبل اعتماده في العمل اليومي.
...
من `factory_permissions` أعط الدور الأفعال التي يحتاجها فقط. لا تعط `view_costs` أو `manage_permissions` لمجرد حل زر مفقود. لواتساب: `manage_whatsapp_session` للاتصال وإنشاء الجلسة، و`edit_whatsapp_messages` لتعديل نصوص إعدادات المعمل. إرسال ملف القياسات أو الفاتورة يبقى على `print_measurements` / `print_customer_invoice`.

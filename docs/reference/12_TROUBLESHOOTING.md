...
- هل حُفظت الخلية المفتوحة؟ الحفظ الصريح ينسخ قيم جدول القياسات قبل الإرسال حتى لو بقي المؤشر داخل الخلية.
- هل اختفى زر حفظ الطلب من أول نقرة؟ إذا بقي ظاهرًا بعد حفظ ناجح فالمشكلة في قفل جلسة التعديل بعد `frm.save()`، لا في المحسّن.
...
إذا ظهرت «خطة القص المعتمدة لا يمكن إعادة حسابها خارج مرحلة الرسم» على طلب **مسودة جديد** فهذا خطأ lifecycle: المسودة قبل الإرسال تبقى قابلة لإعادة الحساب والرفع حتى مع خطة معتمدة أو مسار مخطط. القفل صحيح فقط بعد مغادرة مرحلة الرسم (CNC وما بعدها). لا تُرجع الطلب إلى المسودة كحل إذا كان أصلًا في Draft.
...
2. طبقات `SHEET_OUTLINE` و`CUT_PATH` حسب العقد، أو الطبقة `0` كبديل لـ `CUT_PATH` عندما تكون `CUT_PATH` فارغة. إذا غابت الطبقتان الكانونيتان معاً، تُقرأ المحيطات المغلقة على الطبقة `0` (مستطيل اللوح بمقاس الطلب + الدرف). أي طبقة أخرى مثل `along` و`PIECES` لا تُستخدم كحدود لوح أو مسار قص. طبقة `text` لأرقام الدرف وكتابات Extra double داخل ملف DXF فقط ولا تُعد قصًا ولا تغيّر طباعة الخطة. طبقات Extra (`Liner` / `Rear Groove` / `Handle Recess`) تُقرأ ككيانات DXF أصلية فوق درفة Extra فقط (`LINE` أو مسار مفتوح أو مغلق، حتى لو اقترب المسار من نفسه)، ولا تغني عن طبقات اللوح والقص. فحص التفرع ينطبق على محيطات اللوح والقص فقط. كل خانة Extra مفعّلة في جدول القياسات (لاينر / فرزة ظهر / مسكة غطس) تتطلب علامة واحدة على الطبقة المطابقة داخل تلك الدرفة؛ رفض «لا يحتوي علامة» يعني الرسم ناقص، ورفض «دون تفعيل الخانة» أو «أكثر من علامة» يعني الرسم يزيد عن الطلب. احفظ صف Extra قبل إعادة الرفع.
...
إذا كان الملف المنزّل يختلف عن المرفوع: زر تبويب خطة النظام يولّد `_AutoCAD2020_R12.dxf` من snapshot (`SHEET_OUTLINE`/`CUT_PATH` كـ`LINE`، وأرقام الدرف وكتابات Extra double كـ`TEXT` على طبقة `text`؛ طبقات مثل `along` تُسقط). من تبويب الخطة المرفوعة استخدم **تنزيل DXF المرفوع** ليعيد ملف `Cutting Plan.dxf_file` الأصلي بعد التحقق من ارتباطه بالخطة.

...
2. طبقات `SHEET_OUTLINE` و`CUT_PATH` حسب العقد، أو الطبقة `0` كبديل لـ `CUT_PATH` عندما تكون `CUT_PATH` فارغة. إذا غابت الطبقتان الكانونيتان معاً، تُقرأ المحيطات المغلقة على الطبقة `0` (مستطيل اللوح بمقاس الطلب + الدرف). أي طبقة أخرى مثل `along` و`PIECES` لا تُستخدم كحدود لوح أو مسار قص. طبقات Extra (`Liner` / `Rear Groove` / `Handle Recess`) إن وُجدت تُقرأ ككيانات DXF أصلية فوق درفة Extra فقط (`LINE` أو مسار مفتوح أو مغلق، حتى لو اقترب المسار من نفسه)، ولا تغني عن طبقات اللوح والقص. فحص التفرع ينطبق على محيطات اللوح والقص فقط. علامة Extra تتطلب تفعيل الخانة المطابقة في صف Extra في الطلب (لاينر / فرزة ظهر / مسكة غطس) ثم حفظ الطلب؛ رفض «دون تفعيل الخانة» يعني الطلب لا الرسم.

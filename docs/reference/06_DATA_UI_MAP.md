...
- `factory_production_settings`: إعدادات المصنع المقسمة حسب Capabilities. بطاقة جلسة WhatsApp الحية (إنشاء/حالة/QR/إعادة اتصال) تظهر فقط مع `manage_whatsapp_session` عبر OpenWA دون تخزين المفتاح في DocType. قسم **رسائل واتساب** يُعرض مع إعدادات المعمل ويُعدَّل فقط مع `edit_whatsapp_messages` (نص القياسات لأول إرسال، ونص تعديلات القياسات للإرسال اللاحق، والفاتورة، ورسائل إتمام المراحل المفعّلة على المسار).

...
- `factory_production_settings`: إعدادات المصنع المقسمة حسب Capabilities. بطاقة جلسة WhatsApp الحية (إنشاء/حالة/QR/إعادة اتصال) تظهر فقط مع `manage_whatsapp_session` عبر OpenWA دون تخزين المفتاح في DocType. قسم **رسائل واتساب** يُعرض مع إعدادات المعمل ويُعدَّل فقط مع `edit_whatsapp_messages` (نص القياسات والفاتورة ورسائل إتمام المراحل المفعّلة على المسار).
...
من تاب التكلفة، بجانب `طباعة فاتورة الزبون`، يظهر `إرسال الفاتورة عبر واتساب` (`dco-whatsapp-send-invoice`) للمستخدم الذي يملك `print_customer_invoice` على طلب محفوظ. الضغط يفتح سؤال تأكيد ثم يرسل رسالة نصية مضبوطة من إعدادات المعمل (قسم رسائل واتساب) متبوعة بملف PDF المطابق لطباعة فاتورة الزبون (جدول القياسات + تفاصيل عرض السعر المعتمدة). HTML يُبنى على الخادم من الحمولة المالية المصرّح بها؛ لا يُرسل HTML من المتصفح ولا بيانات التكلفة الداخلية. `{order_name}` في القالب يُستبدل برقم الطلب.

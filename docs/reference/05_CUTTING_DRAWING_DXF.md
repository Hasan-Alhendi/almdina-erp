...
- `Liner` / `Rear Groove` / `Handle Recess` لعلامات Extra فقط (اللاينر وفرزة الظهر ومسكة الغطس). هذه الطبقات ليست `CUT_PATH`: لا تدخل عدد القطع ولا overlap/Kerf بين الأبواب. كل علامة تُقرأ ككيان DXF أصلي (`LINE` أو `LWPOLYLINE` مفتوح أو مغلق)، بما في ذلك المسارات التي تقترب من نفسها مثل مسكة الغطس؛ لا يُطبَّق عليها تجميع التفرع المستخدم لمحيطات القص. ويجب أن تقع كل نقاطها داخل درفة Extra واحدة بعد مطابقة الطلب. خانة Extra المفعّلة تتطلب علامة واحدة على الطبقة المطابقة داخل تلك الدرفة؛ زيادة علامة دون خانة أو نقصان علامة والخانة مفعّلة يرفض الاستيراد. Double / درفة كاملة ليست طبقات DXF: تظهر ككتابة على الدرفة، ويكتب تصدير خطة النظام النصين العربيين `دبل القشاط` و`دبل كامل` داخل محيط الدرفة على طبقة `text`.
- `text` لأرقام الدرف وكتابات Extra double في ملف DXF. التصدير يكتب كيان `TEXT` لرقم كل درفة في مركزها، ويكتب `دبل القشاط` و`دبل كامل` داخل زوايا درفة Extra عند اختيارهما، تحت خط القص لا عليه. الاستيراد يقرأ `TEXT`/`MTEXT` من هذه الطبقة ويحفظها في `sheets[].text_labels` بوحدة `usable_sheet` mm. الطبقة ليست `CUT_PATH` ولا تُستخدم لمطابقة الطلب ولا لتغيير طباعة الخطة. غيابها مسموح.

...
بعد اكتمال التعبئة تُعكس إحداثيات Y داخل المساحة القابلة للاستخدام حتى تتجمع الدرفات أسفل اللوح وتبقى الفضلة أعلاه، ويُعكس ترتيب الألواح بحيث يظهر آخر لوح فُتح أولاً في المعاينة والطباعة وDXF. خطط DXF المرفوعة لا تُعاد توجيهها.
...
عند استيراد DXF، قطعة `Special` تعني **شكلًا حرًا داخل مقاس تصنيع ثابت**. يجوز أن يكون المحيط الخارجي نجمة أو حرف `L` أو شكلًا مقعرًا أو أي Polygon مغلق صالح، ولا يشترط أن يكون مستطيلاً. لكن `Bounding Box` للمحيط الخارجي يجب أن يطابق `cut_width_cm × cut_length_cm` المحفوظين للقطعة ضمن عقد الاستيراد، أو يطابقهما بعد التدوير فقط عندما يكون `allow_rotation` مفعّلًا. لا يجوز أن تتحول حرية الشكل إلى تجاوز لمقاس القص التصنيعي. تبقى الفتحات الداخلية مملوكة لمحيط خارجي واحد، وإذا لم تستطع أدلة الطلب والبنية الهندسية التمييز يقينًا بين فتحة وقطعة مستقلة يفشل الاستيراد صراحة بدل التخمين. علامات تصنيع درفة Extra (اللاينر وفرزة الظهر ومسكة الغطس) تُرسم على طبقات مستقلة `Liner` و`Rear Groove` و`Handle Recess` فوق درفة Extra، ولا تُعد قطع قص ولا توضع على `CUT_PATH`. الخانة المفعّلة في جدول القياسات تتطلب علامة واحدة مطابقة على الطبقة داخل تلك الدرفة، والعلامة المرسومة دون تفعيل الخانة تُرفض. العلامة قد تكون خطًا أو مسارًا مفتوحًا أو مضلعًا مغلقًا؛ المطلوب أن تقع كل نقاطها داخل درفة Extra واحدة بعد مطابقة الطلب. طباعة خطة القص تُبقي علامات Extra في مواضعها الهندسية: فرزة الظهر كخط متقطع أخضر أنحف، واللاينر كخط متقطع واحد أزرق بنفس الأسلوب، ومسكة الغطس كمستطيل بخطوط رفيعة في حدود العلامة، وتضيف رمزي دبل القشاط ودبل كامل الدرفة من خانات الطلب (بلا مسار DXF)، مع دليل رموز لكل الإضافات الخمس في رأس كل صفحة ألواح جاهزة للطباعة.
...
المستخدم يحتاج Capabilities مختلفة للعرض/إعادة الحساب/الرفع/الاعتماد. اعتماد الخطة هو نقطة تثبيت لما سيغادر مرحلة التخطيط إلى الإنتاج، ويثبّت هندسة القص دون أن يشترط سعر اللوح أو أجور القص أو اعتماد أسعار الدرف الخاصة؛ تلك تبقى على تبويب التكلفة والمستندات المالية. تبويب **الخطة المعتمدة** يعكس علاقة `Door Cutting Order.approved_plan` الحالية فقط؛ فتح التاب لا يعتمد خطة، ولا تُعرض خطة Approved يتيمة أو تابعة لطلب آخر كاعتماد لهذا الطلب. تبويب **الخطة المرفوعة** يبقي آخر خطة DXF مرفوعة ظاهرة حتى بعد اعتمادها أو إلغاء ذلك الاعتماد؛ مسودة رفع أحدث تحل محلها عند وجودها. أثناء جلسة تعديل إعدادات خطة النظام تبقى تبويبات **الخطة المرفوعة** و**الخطة المعتمدة** معطّلة حتى الحفظ أو الإلغاء، حتى لو أُعيد رسم مساحة العمل؛ القفل جزء من رسم شريط المصدر نفسه وليس خطوة لاحقة قابلة للسباق.

حفظ صريح لأي مدخل يؤثر على توزيع القطع (قياسات الدرف، الكمية، النوع، أضلاع/سماكة القشاط، مقاس اللوح، الدوران، الأشكال الخاصة) يبقي طلب الحفظ سريعًا: يثبّت الصفوف ويعلّم مسودة خطة النظام بأنها قديمة، ويلغي اعتماد خطة الإنتاج الحالية إذا لم تعد مطابقة، دون تحويل هندستها إلى مسودة. اسم الزبون ونوع اللوح ولون القشاط لا تدخل هذا العقد. قبل ذلك الحفظ تُنسخ قيم الخلايا المفتوحة في جدول القياسات إلى النموذج حتى لا تُفقد إذا بقي المؤشر داخل الخلية، ويُقفل وضع التعديل من نفس النقرة حتى لو تأخر `after_save` في Frappe. إذا كان المستخدم يملك `recalculate_plan` والحالة تسمح بإعادة الحساب، يُجدول حساب خطة النظام في طابور `long` ثم تُحفظ الخطة وتكلفة الألواح/القص تلقائيًا عند اكتمال العامل. الواجهة لا تُجمَّد. Checkpoint الداخلي قبل زر إعادة الحساب/المعاينة لا يطلق هذا المسار. خطة DXF المرفوعة لا يُعيد المحسّن توليدَها؛ تُعلَّم فقط بأنها قديمة ويظهر فوق رسمها بانر واضح أن الخطة غير مطابقة للطلب لأن بيانات القص تغيّرت. زر إعادة الحساب اليدوي يبقى لمسار الإعدادات والمعاينة وإعادة المحاولة إذا تعثر العامل الخلفي.
...
- هندسة الخطة المعتمدة تبقى Snapshot غير قابلة للتعديل. أي تغيير لاحق لا يعيد كتابة ذلك المستند كمسودة.
- حفظ مدخلات تؤثر على توزيع القطع يلغي علاقة الاعتماد الحالية (`Approved` → `Cancelled`) ويفرغ `Door Cutting Order.approved_plan` ضمن نفس حفظ الطلب، دون كتابة صف الأب مرة ثانية بعد تحديث `modified`. الإنتاج يحتاج اعتمادًا جديدًا بعد مراجعة الخطة المحدّثة.
- تبويب الخطة المرفوعة يحتفظ بآخر DXF مقبول للعرض حتى بعد الاعتماد أو إلغائه. إذا تغيّرت مدخلات القص (قياسات/عدد/قشاط مؤثر على القص/مقاس اللوح/إعدادات المحسّن) يظهر فوق رسم الخطة بانر واضح: «الخطة غير مطابقة للطلب / بيانات هذا الطلب تم تعديلها وأصبحت الخطة غير مطابقة». تغيير اسم الزبون أو نوع اللوح أو لون القشاط لا يلغي الاعتماد ولا يظهر هذا البانر.
- في مرحلة الرسم/التخطيط يمكن رفع DXF بديل أو إعادة حساب خطة النظام ثم إعادة الاعتماد.
- بعد مغادرة مرحلة الرسم لا يعاد تشغيل optimizer تلقائيًا لعرض تاريخ قديم.
...
- `SHEET_OUTLINE`
- `CUT_PATH`
- الطبقة `0` كبديل لـ `CUT_PATH` عندما تكون `CUT_PATH` فارغة. إذا وُجدت `SHEET_OUTLINE` فكل محيط مغلق على الطبقة `0` (عدا صناديق تأطير علامات Extra) مسار قص. إذا غابت `SHEET_OUTLINE` أيضاً، مستطيل اللوح بمقاس الطلب على الطبقة `0` يصبح حدود اللوح وباقي الدرف مسارات قص.
- `Liner` / `Rear Groove` / `Handle Recess` لعلامات Extra فقط (اللاينر وفرزة الظهر ومسكة الغطس). هذه الطبقات ليست `CUT_PATH`: لا تدخل عدد القطع ولا overlap/Kerf بين الأبواب. كل علامة تُقرأ ككيان DXF أصلي (`LINE` أو `LWPOLYLINE` مفتوح أو مغلق)، بما في ذلك المسارات التي تقترب من نفسها مثل مسكة الغطس؛ لا يُطبَّق عليها تجميع التفرع المستخدم لمحيطات القص. ويجب أن تقع كل نقاطها داخل درفة Extra واحدة بعد مطابقة الطلب. خانة Extra المفعّلة تتطلب علامة واحدة على الطبقة المطابقة داخل تلك الدرفة؛ زيادة علامة دون خانة أو نقصان علامة والخانة مفعّلة يرفض الاستيراد. Double / درفة كاملة ليست طبقات DXF: تظهر ككتابة على الدرفة، ويكتب تصدير خطة النظام نصين إنجليزيين واضحين داخل محيط الدرفة على طبقة `text`: `Double Edge Banding` لدبل القشاط و`Full Door Double` لدبل كامل الدرفة.
- `text` لأرقام الدرف وكتابات Extra double في ملف DXF. التصدير يكتب كيان `TEXT` لرقم كل درفة في مركزها، ويكتب `Double Edge Banding` و`Full Door Double` داخل زوايا درفة Extra عند اختيارهما، تحت خط القص لا عليه. الاستيراد يقرأ `TEXT`/`MTEXT` من هذه الطبقة ويحفظها في `sheets[].text_labels` بوحدة `usable_sheet` mm. الطبقة ليست `CUT_PATH` ولا تُستخدم لمطابقة الطلب ولا لتغيير طباعة الخطة. غيابها مسموح.

إذا غابت `CUT_PATH`، تُقرأ المحيطات المغلقة على الطبقة `0` كمسارات قص (نفس دور `CUT_PATH`). إذا غابت `SHEET_OUTLINE` أيضاً، مستطيل اللوح بمقاس الطلب على الطبقة `0` يصبح حدود اللوح وباقي الدرف مسارات قص. طبقات `along` و`PIECES` تبقى للعرض عند المصمم ولا تُعد قصًا. طبقة `text` لأرقام الدرف وكتابات Extra double داخل DXF فقط. الصناديق المغلقة على الطبقة `0` التي تؤطر علامات Extra فوق درفة أكبر لا تُحسب قطعًا إضافية. إذا وُجدت `CUT_PATH` فهي المرجع لمسارات القص ولا تُخلط مع الطبقة `0`.
...
- طباعة الألواح تعرض مسارات Extra في مواضعها: فرزة ظهر بخط متقطع أخضر أنحف، ولاينر بخط متقطع واحد أزرق، ومسكة غطس كمستطيل بخطوط رفيعة، مع كتابة `دبل قشاط` و`دبل كامل الدرفة` على زاويتي درفة Extra بدل الرموز، ودليل الإضافات أعلى شبكة الألواح في كل صفحة طباعة.

...
المستخدم يحتاج Capabilities مختلفة للعرض/إعادة الحساب/الرفع/الاعتماد. اعتماد الخطة هو نقطة تثبيت لما سيغادر مرحلة التخطيط إلى الإنتاج، ويثبّت هندسة القص دون أن يشترط سعر اللوح أو أجور القص أو اعتماد أسعار الدرف الخاصة؛ تلك تبقى على تبويب التكلفة والمستندات المالية. تبويب **الخطة المعتمدة** يعكس علاقة `Door Cutting Order.approved_plan` الحالية فقط؛ فتح التاب لا يعتمد خطة، ولا تُعرض خطة Approved يتيمة أو تابعة لطلب آخر كاعتماد لهذا الطلب. أثناء جلسة تعديل إعدادات خطة النظام تبقى تبويبات **الخطة المرفوعة** و**الخطة المعتمدة** معطّلة حتى الحفظ أو الإلغاء، حتى لو أُعيد رسم مساحة العمل؛ القفل جزء من رسم شريط المصدر نفسه وليس خطوة لاحقة قابلة للسباق.

حفظ صريح لأي مدخل يؤثر على توزيع القطع (قياسات الدرف، الكمية، النوع، القشاط، مقاس اللوح، الدوران، الأشكال الخاصة) يبقي طلب الحفظ سريعًا: يثبّت الصفوف ويعلّم مسودة خطة النظام بأنها قديمة. قبل ذلك الحفظ تُنسخ قيم الخلايا المفتوحة في جدول القياسات إلى النموذج حتى لا تُفقد إذا بقي المؤشر داخل الخلية، ويُقفل وضع التعديل من نفس النقرة حتى لو تأخر `after_save` في Frappe. إذا كان المستخدم يملك `recalculate_plan` والحالة تسمح بإعادة الحساب، يُجدول حساب خطة النظام في طابور `long` ثم تُحفظ الخطة وتكلفة الألواح/القص تلقائيًا عند اكتمال العامل. الواجهة لا تُجمَّد. Checkpoint الداخلي قبل زر إعادة الحساب/المعاينة لا يطلق هذا المسار. خطة DXF المرفوعة لا يُعيد المحسّن توليدَها؛ تُعلَّم فقط بأنها قديمة. زر إعادة الحساب اليدوي يبقى لمسار الإعدادات والمعاينة وإعادة المحاولة إذا تعثر العامل الخلفي.
...
- في **المسودة قبل الإرسال للإنتاج** يمكن إعادة الحساب أو رفع DXF أو تعديل إعدادات القص حتى مع وجود خطة معتمدة؛ الاعتماد السابق لا يُفعَّل في الإنتاج حتى يُعتمد البديل بعد المراجعة.
- في مرحلة الرسم/التخطيط يمكن إعداد بديل ثم إعادة الاعتماد. Snapshot المعتمد الحالي يبقى مرجع الإنتاج حتى ذلك الحين.
- بعد مغادرة مرحلة الرسم لا يعاد تشغيل optimizer تلقائيًا لعرض تاريخ قديم، والخطة المعتمدة تبقى مقفلة.
- أي تغيير Geometry جوهري لاحق في المسودة يعلّم خطة النظام بأنها قديمة، ثم يعيد حسابها في الخلفية عند الحفظ الصريح إذا توفرت صلاحية `recalculate_plan`، دون تشغيل المحسّن داخل طلب الحفظ نفسه.

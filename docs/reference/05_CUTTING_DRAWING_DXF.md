...
عند استيراد DXF، هوية قطعة `Special` هي **محيط خارجي حر صالح للقص** وليست مستطيلاً ولا تطابق `Bounding Box` إلزاميًا مع العرض والطول الاسميين. يمكن أن يكون المحيط نجمة أو حرف `L` أو وردة أو أي Polygon مغلق صالح. تبقى القطع Regular/Clipped/Extra مرتبطة بمقاسات القص التصنيعية، وتبقى الفتحات الداخلية مملوكة لمحيط خارجي واحد. إذا لم تستطع أدلة الطلب والبنية الهندسية التمييز يقينًا بين فتحة وقطعة مستقلة موضوعة داخلها، يفشل الاستيراد صراحة بدل التخمين.

...
try {
    $health = Invoke-RestMethod -Method Get -Uri "http://127.0.0.1:$port/health" -Headers @{ "X-Almdina-Scanner-Bridge" = "1" } -TimeoutSec 3
    ...
    if ([int]$health.device_count -gt 0) {
        Write-Host "تم العثور على $($health.device_count) Scanner عبر WIA." -ForegroundColor Green
    }
    else {
        Write-Host "البرنامج يعمل، لكن Windows لا يرى أي Scanner عبر WIA حاليًا." -ForegroundColor Yellow
        Write-Host "ثبّت تعريف الـScanner وتأكد أنه يظهر في Windows ثم أعد المحاولة من الموقع."
    }
}
catch {
